...
from sqlalchemy import create_engine, Column, String, Text, DateTime, Float, JSON, Integer
from sqlalchemy.ext.declarative import declarative_base
from sqlalchemy.orm import sessionmaker, Session

# Database setup
DATABASE_URL = os.getenv('DATABASE_URL', 'postgresql://lucidia:lucidiapassword@postgres:5432/lucidia')
...
class FactCreate(BaseModel):
    ...
    confidence: Optional[float] = Field(1.0, description="Confidence score between 0 and 1.")
    evidence: Optional[Dict[str, Any]] = Field(None, description="Optional evidence metadata for the fact.")
...
def get_knowledge(limit: Optional[int] = 100, db: Session = Depends(get_db)) -> List[Fact]:
...
async def ask_question(req: AskRequest, db: Session = Depends(get_db)) -> AskResponse:
    ...
    try:
        planner_resp = requests.post(planner_url, json={"question": question, "context": {}})
        planner_resp.raise_for_status()
        planner_result = planner_resp.json()
    except Exception as ex:
        raise HTTPException(status_code=502, detail=f"Planner service unavailable: {ex}")
    ...
    answer_text = planner_result.get('answer') or planner_result.get('conclusion', {}).get('answer') or "No answer returned."
